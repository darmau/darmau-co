<script lang="ts">
	...
	const ogImage = $derived(
		`${data.prefix}/cdn-cgi/image/format=jpeg,width=960/${data.category?.cover?.storage_key ?? 'a2b148a3-5799-4be0-a8d4-907f9355f20f'}`
	);
</script>

<svelte:head>
	<title>{homeLabel.category_article_title}</title>
	<!-- 原 meta() 在拿不到分类时只输出 title，这里保持一致 -->
	{#if data.category}
		<meta name="description" content={homeLabel.category_article_description} />
		<link
			rel="alternate"
		...
		/>
		<meta property="og:title" content={homeLabel.category_article_title} />
		<meta property="og:type" content="article" />
		<meta
			property="og:url"
			content="{data.baseUrl}/{lang}/articles/category/{data.category.slug}/{data.page}"
		/>
		<meta property="og:image" content={ogImage} />
		<meta property="og:description" content={homeLabel.category_article_description} />
...
{#if data.category}
	<I18nHead
		...
		path="articles/category/{data.category.slug}/{data.page}"
	/>
{/if}

{#if data.articles.length === 0 || data.category === null}
...
{:else}
	...
	<header class="relative w-full max-w-6xl mx-auto px-4 my-8 space-y-4">
		<div
			class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 text-white text-center space-y-4"
		>
			<h1 class="font-medium text-3xl md:text-5xl shadow-2x">{data.category.title}</h1>
		...
		{#if data.category.cover}
			<div class="relative overflow-hidden rounded-2xl w-full aspect-3/2 md:aspect-3/1">
				<div class="absolute inset-0 bg-linear-to-b from-transparent to-zinc-800/60"></div>
				<ResponsiveImage
					image={data.category.cover}
					width={640}
