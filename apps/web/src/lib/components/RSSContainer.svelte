<script module lang="ts">
	// 原 app/components/RSSContainer.tsx 里其实没有 React 组件，只有一组把 Tiptap JSON
	// 渲染成带内联样式的 HTML 字符串的纯函数（给 RSS feed 用）。这里保持同名文件，
	// 逻辑全部放在 module script 里，用 `import { contentToHtml } from '$components/RSSContainer.svelte'` 引用。
	...
	function renderImage(node: Content, prefix: string): string {
		...
		const captionHtml = caption
			? `<figcaption style="margin-top: 0.5rem; text-align: center; font-size: 0.875rem; color: #6b7280;">${escapeHtml(caption)}</figcaption>`
			: '';
	...
	function renderTable(content?: ContentItem[], prefix?: string): string {
		...
		const header = headerRow
			? `<thead><tr>${renderTableRow(headerRow, true, imagePrefix)}</tr></thead>`
			: '';
		// 注意：原实现在 <tr ...> 里多写了一个 `}`（`style="…";}>`），照抄以保持输出一致
		const body = `<tbody>${bodyRows
			.map(
				(row, index) =>
					`<tr style="background-color: ${index % 2 === 0 ? 'white' : '#f9fafb'};"}>${renderTableRow(row, false, imagePrefix)}</tr>`
			)
			.join('')}</tbody>`;
	...
	function renderTableRow(row: ContentItem, isHeader: boolean, prefix: string): string {
		...
		return (
			row.content
				?.map((cell) => `<${tag} style="${style}">${renderContent(cell.content, prefix)}</${tag}>`)
				.join('') ?? ''
		);
	}
